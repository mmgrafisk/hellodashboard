...
export async function createCategory(name){ return addDoc(collection(state.db, ...path()), { name, createdAt: serverTimestamp() }); }
export async function renameCategory(id, name){ return updateDoc(doc(state.db, ...path(), id), { name }); }
export async function removeCategory(id){ return deleteDoc(doc(state.db, ...path(), id)); }
